...
public class NoteEditActivity extends  AppCompatActivity  { 
	...
	private HashMap<String, Object> GetNote = new HashMap<>();
				...
				if (title_edittext.getText().toString().equals("") && note_edittext.getText().toString().equals("")) {
					...
					if (title_edittext.getText().toString().equals("") && !note_edittext.getText().toString().equals("")) {
						...
						if (note_edittext.getText().toString().equals("") && !title_edittext.getText().toString().equals("")) {
							...
							Notes_List.get((int)Position).put("Note", note_edittext.getText().toString());
							Notes_List.get((int)Position).put("Title", title_edittext.getText().toString());
							if (importantcheckbox.isChecked()) {
								Notes_List.get((int)Position).put("Important", "True");
							}
							else {
								Notes_List.get((int)Position).put("Important", "False");
							}
							Notes_List.get((int)Position).put("Date", Date);
							Notes_List.get((int)Position).put("Time", Time);
	...
	public void _GetSettingsData () {
		...
		if (Settings.getString("Theme", "").equals("Default")) {
			Window w = NoteEditActivity.this.getWindow();w.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);w.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS); w.setStatusBarColor(Color.parseColor("#FFFFFF")); main.setSystemUiVisibility(View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
			_Add("#000000", back_img);
		}
		else {
			if (Settings.getString("Theme", "").equals("Dark")) {
				Window w = NoteEditActivity.this.getWindow();w.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);w.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS); w.setStatusBarColor(Color.parseColor("#000000"));
	...
	public void _GetNoteData () {
		GetNote = Notes_List.get((int)Position);
		title_edittext.setText(GetNote.get("Title").toString());
		note_edittext.setText(GetNote.get("Note").toString());
		characters.setText(String.valueOf((long)(note_edittext.getText().toString().length())).concat(" Characters"));
		if (GetNote.get("Important").toString().equals("True")) {
